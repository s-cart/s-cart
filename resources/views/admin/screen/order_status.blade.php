          ...
          <div class="box-body no-padding" >
            <form action="{{ $url_action }}" method="post" accept-charset="UTF-8" class="form-horizontal" id="form-main">
                  ...
                  <div class="form-group   {{ $errors->has('name') ? ' has-error' : '' }}">
                      <label for="name" class="col-sm-2 col-form-label">{{ trans('order_status.name') }}</label>
                      <div class="col-sm-8">
                          <div class="input-group">
                              <span class="input-group-addon"><i class="fa fa-pencil fa-fw"></i></span>
                              <input type="text" id="name" name="name" value="{!! old()?old('name'):$order_status['name']??'' !!}" class="form-control name" placeholder="" />
                          </div>
                              @if ($errors->has('name'))
      ...
      <div class="box-header with-border">
        ...
        <div class="box-tools">
            <div class="btn-group pull-right" style="margin-right: 5px">
                <a href="{{ route('admin_order_status.index') }}" class="btn  btn-flat btn-default" title="List"><i class="fa fa-list"></i><span class="hidden-xs"> {{trans('admin.back_list')}}</span></a>
            </div>
        </div>
        @endif
      </div>
            ...
            $.ajax({
                ...
                success: function (data) {
                    if(data.error == 1){
                      ...
                      window.location.replace('{{ route('admin_order_status.index') }}');
                    }
