          ...
          <div class="box-body no-padding" >
            <form action="{{ $url_action }}" method="post" accept-charset="UTF-8" class="form-horizontal" id="form-main">
                    ...
                    <div class="form-group   {{ $errors->has('name') ? ' has-error' : '' }}">
                      ...
                      <div class="col-sm-8">
                          <div class="input-group">
                              ...
                              <input type="text" id="name" name="name" value="{!! old('name',($attribute_group['name']??'')) !!}" class="form-control name" placeholder="" />
                          </div>
                              @if ($errors->has('name'))
                  ...
                  <div class="form-group  {{ $errors->has('type') ? ' has-error' : '' }}">
                      ...
                      <div class="col-sm-8">
                          <label class="radio-inline"><input type="radio" name="type" value="radio" {{ (old('type',($attribute_group['type']??'')) =='radio')?'checked':'' }}>Radio</label>
                          <label class="radio-inline"><input type="radio" name="type" value="select" {{ (old('type',($attribute_group['type']??'')) =='select')?'checked':'' }}>Select</label>
                              @if ($errors->has('type'))
                                  <span class="help-block">
                                      {{ $errors->first('type') }}
                                  </span>
            ...
            $.ajax({
                ...
                success: function (data) {
                    if(data.error == 1){
                      ...
                      window.location.replace('{{ route('admin_attribute_group.index') }}');
                    }
