        ...
        <div class="row">
              ...
              @if (!empty($layoutsUrl['footer']))
                @foreach ($layoutsUrl['footer'] as $url)
                  <li class="mb-3 nav-item">
                    <a class="nav-link" {{ ($url->target =='_blank')?'target=_blank':''  }} href="{{ sc_url_render($url->url) }}">
          ...
          <div class="col-12 col-sm-8 mt-6 mt-sm-0 navbar-dark"">
            ...
            <div class="subscribe-form">
              <form id="mc-form" action="{{ route('subscribe') }}" method="post" class="searchform row align-items-center no-gutters mb-3">
                @csrf
                <div class="col">
                  <input type="email" name="subscribe_email" class="email form-control input-2 bg-white" id="mc-email" required="required" placeholder="{{ trans('front.subscribe.subscribe_email') }}">
                </div>
                <div class="col-auto">
                  <input class="btn btn-primary overflow-auto" name="subscribe" value="Subscribe" type="submit">
                </div>
              </form>  
              <p class="nav-link">{{ trans('front.subscribe.subscribe_des') }}</p>
            </div>
          </div>
        </div>
      </div>
      <div class="col-12 col-lg-3 mt-6 mt-lg-0">
        <div class="d-flex mb-3">
          ...
          <div>
            ...
            <p class="mb-0 text-muted">{{ trans('front.shop_info.address') }}: {{ sc_store('address') }}</p>
          </div>
        </div>
        <div class="d-flex mb-3">
          ...
          <div>
            ...
            <a class="text-muted" href="mailto:themeht23@gmail.com"> {{ trans('front.shop_info.email') }}: {{ sc_store('email') }}</a>
          </div>
        </div>
        <div class="d-flex mb-3">
          ...
          <div>
            ...
            <a class="text-muted" href="tel:+{{trans('front.shop_info.hotline')}}">{{ trans('front.shop_info.hotline') }}: {{ sc_store('long_phone') }}</a>
          </div>
        </div>
        <div class="d-flex">
...
<!-- Subscribe Modal -->
<div class="modal fade" id="mailchimpModal-disabled" tabindex="-1" role="dialog" aria-labelledby="ModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
